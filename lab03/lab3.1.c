...
void *studentFunction(void *param) {
    ...
    // sleep a random time between 1 and 10
    sleep(rand() % 10 + 1);
    printf("student %d arrived at the queue\n", i);
    st_info.id = i;
    st_info.office_no = -1;
    st_info.urgent = 0;
    // critical section
    ...
    printf("student %d has been selected from office %d\n", i, office_number);
    ...
    if (response.office_no != -1) {
        ...
        printf("student %d going from normal office %d to the special office\n", i, office_number);
        ...
        printf("student %d served by the special office, going back to office %d\n", i, office_number);
        st_info.id = i;
        st_info.office_no = office_number;
...
void *officeFunction(void *param) {
    ...
    int i = *office_no;
    Info info, receiveMsg, officeAnswer;
    pthread_detach(pthread_self());
    while(1) {
        pthread_mutex_lock(&cond->lock);
